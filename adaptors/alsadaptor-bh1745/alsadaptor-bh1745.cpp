...
ALSAdaptorBh1745::ALSAdaptorBh1745(const QString& id) : SysfsAdaptor(id, SysfsAdaptor::IntervalMode)
{
    memset(buf, 0x0, 16);
...
ALSAdaptorBh1745::~ALSAdaptorBh1745()
{
    delete alsBuffer_;
}

void ALSAdaptorBh1745::processSample(int pathId, int fd) {
    Q_UNUSED(pathId);
    ...
    sensordLogT() << "Ambient light value:" << idata << buf;
...
bool ALSAdaptorBh1745::startSensor()
{
...
void ALSAdaptorBh1745::stopSensor()
{
    if(!powerStatePath.isEmpty()) {
